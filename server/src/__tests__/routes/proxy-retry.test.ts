...
describe('isRetryableError', () => {
  ...
  describe('422 Unprocessable Entity (fallback feature)', () => {
    it('treats explicit "422" in the error message as retryable', () => {
      expect(isRetryableError(new Error('Mistral API error 422: Unprocessable Entity'))).toBe(true);
      expect(isRetryableError(new Error('Provider error 422: Unprocessable'))).toBe(true);
    });

    it('treats unprocessable phrasing as retryable', () => {
      expect(isRetryableError(new Error('unprocessable entity'))).toBe(true);
      expect(isRetryableError(new Error('unprocessable parameter schema'))).toBe(true);
    });
  });
