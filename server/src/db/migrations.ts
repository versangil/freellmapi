...
function ensurePlaygroundSessionsThinkingColumn(db: Database.Database) {
  try {
    const columns = db.prepare("PRAGMA table_info(playground_sessions)").all() as { name: string }[];
    if (columns.length > 0 && !columns.some(col => col.name === 'thinking')) {
      db.prepare("ALTER TABLE playground_sessions ADD COLUMN thinking TEXT NOT NULL DEFAULT 'medium'").run();
    }
  } catch (err) {
